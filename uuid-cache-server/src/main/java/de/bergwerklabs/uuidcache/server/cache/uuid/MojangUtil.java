 ...
 *
 * <p>Contains methods for resolving name to {@link UUID}s and vice versa.
 *
...
public class MojangUtil {
  ...
  public static Optional<String> nameForUuid(UUID uuid) {
    ...
    try {
      URLConnection connection =
          retrieveConnection("https://api.mojang.com/user/profiles/" + shortUuid + "/names");
      JsonArray array =
          PARSER
              .parse(new String(ByteStreams.toByteArray(connection.getInputStream()), "UTF-8"))
              .getAsJsonArray();
  ...
  public static Optional<PlayerNameToUuidMapping> uuidForName(String name) {
    try {
      ...
      URLConnection connection =
          retrieveConnection("https://api.mojang.com/users/profiles/minecraft/" + name);
      JsonElement element =
          PARSER.parse(new String(ByteStreams.toByteArray(connection.getInputStream()), "UTF-8"));
      if (element.isJsonNull()) return Optional.empty();
      JsonObject object = element.getAsJsonObject();
      return Optional.of(
          new PlayerNameToUuidMapping(
              object.get("name").getAsString(), toLongUuid(object.get("id").getAsString())));
    } catch (Exception e) {
      e.printStackTrace();
    }
    return Optional.empty();
  ...
  private static URLConnection retrieveConnection(String url) throws Exception {
    ...
    connection.connect();
    // Aborts connection attempt if it takes longer than 500ms (To prevent problems when the mojang
    // WS is down)
    connection.setConnectTimeout(500);
    // If the connection was made, but the service takes more than 500ms to complete its answer,
    // abort the request
   ...
   *
   * <p>For example the short id {@code 92de217b8b2b403b86a5fe26fa3a9b5f} would be converted to
   * {@code 92de217b-8b2b-403b-86a5-fe26fa3a9b5f}.
   *
   * @param shortUuid {@link UUID} without the hyphens. {@code 92de217b8b2b403b86a5fe26fa3a9b5f} for
   *     example.
  ...
  private static UUID toLongUuid(String shortUuid) {
    ...
    if (matcher.matches()) {
      ...
      System.out.println(
          "Could not convert short UUID " + shortUuid + " to long UUID. Returning null...");
    return null;
  }
}
